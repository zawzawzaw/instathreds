  ...
  <tr bgcolor="#ffffff">
    <td colspan="2">
      <div style="padding:20px;">
        ...
        @if($redemption_type=="shipping")
	        @if($shipping_method=='shipmethod-standard') 
	        	{{ "Standard Shipping (5-15 business days)" }}
	        @elseif($shipping_method=='shipmethod-express') 
	        	{{ "Express Shipping (3-7 Business Days)" }}
	        @else
				    {{ "International Shipping" }}
		      @endif
    		@else
    			{{ 'Collection at '.$store_location }}
    		@endif
        <br>
		<?php $i = 1; ?>
        @foreach($order_items as $order_item)
	        ({{ $i }}) {{ $order_item->name }} - {{ ucwords($order_item->options->gender).' '.$order_item->options->shirt_type }} - {{ $order_item->qty }} - ${{ $order_item->price }}<br>
	        <?php $i++; ?>
        @endforeach

        --------------------------------------------------<br>
        Item(s) cost: ${{ number_format((float)$items_cost, 2, '.', '') }}<br>
        Shipping cost: ${{ number_format((float)$shipping_cost, 2, '.', '') }}<br>
        --------------------------------------------------<br>
        Subtotal: ${{ number_format((float)$sub_total, 2, '.', '') }}<br>
        Discount: - ${{ number_format((float)$discount_amount, 2, '.', '') }}<br>
        --------------------------------------------------<br>
        - ${{ number_format((float)$total, 2, '.', '') }} (Stripe)<br>
        --------------------------------------------------<br>
        Total cost: ${{ number_format((float)$total, 2, '.', '') }}
        </p>
